...
for library_csv in "${project_dir}/IPS_scripts/libraries/"*; do
    ...
    output_file="${project_dir}/IPS_outs/${library_id}/cellbender/output_posterior.h5"
...
#SBATCH --output "${project_dir}/IPS_outs/logs/${library_id}_cellbender.out"
#SBATCH --error "${project_dir}/IPS_outs/logs/${library_id}_cellbender.out"
#SBATCH --ntasks 1
...
cd "${project_dir}/IPS_outs/${library_id}"
mkdir -p cellbender
